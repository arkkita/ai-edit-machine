...
| OpenAI `gpt-5.6-luna` | $1.00/M input; $0.10/M cached input; $6.00/M output | [OpenAI Luna](https://developers.openai.com/api/docs/models/gpt-5.6-luna) |
...
The OpenAI Luna model page and central pricing table were rechecked on 2026-08-15 after a live M1 run exposed that the earlier bundled `$0.20/$1.20` card was stale. The current `$1/$6` rates above now agree. Production still requires a fresh immutable card and resolved-model preflight rather than treating this document as executable pricing.
...
The M1 OpenAI verifier reserves at most nine built-in web-search calls and prices all nine before consent. The one-call margin follows an observed response that exposed one more web-search output record than requested; anything above the nine-call reservation still fails closed. A live multi-candidate response repeatedly stopped after five optional searches despite being asked to cover eight immutable TVmaze seeds. The corrected path therefore makes eight candidate-scoped Responses requests, each with one immutable episode seed, `tool_choice="required"`, and a one-search request ceiling. Assistant prose is discarded; the host consumes only each hosted tool's complete source list and then applies its own page checks. The verifier may additionally make at most thirty-two credential-free public-page checks (forty total HTTP requests); those checks are candidate-balanced and remain HTTPS-only, DNS-pinned, exact-host, byte-bounded, and subject to the same source/date/identity validation. The [official OpenAI Web Search guide](https://developers.openai.com/api/docs/guides/tools-web-search) documents that `tool_choice="auto"` makes search optional, that `required` should be used when search must run, and that `web_search_call.action.sources` is the complete list of consulted URLs rather than only inline citations. The verifier also sends `search_context_size="low"`; OpenAI documents that this controls how much search-result context is made available, but does not promise an exact token or source count. A live M1 response demonstrated why these limits must be separate: a request body bounded below 30,000 conservative tokens produced 46,828 provider-reported input tokens after five web searches because provider-added search context was billable input. The verifier therefore keeps a 60,000-token aggregate request-body preflight while separately reserving up to 120,000 provider-reported input tokens, 16,000 aggregate output tokens (2,000 for each candidate-scoped call), and nine searches. The larger output allowance follows a measured candidate call that returned typed `incomplete: max_output_tokens` at the former 750-token slice because reasoning tokens consumed the same ceiling. At the current Luna rate the verifier maximum is **$0.3060**. The synthesis capability reserves 30,000 input plus 8,000 output tokens for the first attempt and the same **$0.0780** token allowance for its one syntax-only repair, for a **$0.1560** synthesis maximum. The complete M1 cloud maximum is therefore **$0.4620**, still below the default $0.50 per-run hard cap; the additional public-page checks carry no provider charge but remain request- and time-bounded.

The 120,000-token value is a conservative accounting ceiling calibrated from observed usage with margin, not a claim that `search_context_size` hard-caps billed input. Opaque provider call IDs are stored only as bounded SHA-256-derived audit labels. A response that reports usage beyond any reserved dimension is still reconciled as an overrun and held conservatively; it is not discarded as malformed, silently undercounted, or allowed to authorize a later call. Historical ledger entries remain bound to the immutable price card that authorized them; discovering a newer card does not silently rewrite prior reservations or actuals. The earlier one-call `$0.0912` verifier diagnostic authorization is exhausted and is not reusable under the corrected card.
