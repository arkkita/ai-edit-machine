...
| `IntentInterpretation` (canonical contract) | Versioned hard/soft/audience/platform/creative facets, chip labels/rationales/removability, semantic research questions, broad/clarification state, direct-TikTok-data flag, and mandatory short-form inference disclaimer. It is persisted inside the immutable intent/result contract rather than flattened into untyped query text. |
| `CandidateFunnel` (canonical result) | Fourteen named stage counts from parsing through UI display, bounded rejection-reason counts, recovery-attempt outcome, shortage/broadening explanations, and bounded candidate-level diagnostics. Each deeply researched title records its shortlist reason, retained hook/audience/fandom/story evidence, source categories and evidence IDs, score/threshold traces (`NOT_COMPUTED` when not reached), exact gate, and retrieval/evidence/threshold classification. Development replay adds provider/model/prompt and cost/latency evidence without storing credentials or unrestricted bodies. |
...
| `OpportunityQualityScore` / `ShortFormEditPotential` (canonical result) | Selected profile/version, ten component values and final score; separately, a named inferred short-form band with cited signals and the direct-data disclaimer. No probability field exists. |
| `FandomStoryDossier` (canonical result) | Exactly one evidence-to-concept packet per opportunity: current hook, named characters/relationship, current source kind and locator, verified or inferred quote leads, canonical/franchise/history connections, why fans care, audience/fandom support, uncertainties, and evidence references. Every concept must cite this dossier and may use only its evidence. |
...
| `footage_request` | Opportunity and selected concept identity, immutable revision, conversational summary, minimum-useful-set explanation, preferred source class, spoiler scope, `BEST`/`ALTERNATIVE`/`MINIMUM`/`OPTIONAL_IMPROVEMENT` presentation text, generated provenance. One request can span many episodes, seasons, trailers, clips, or a scene pack. No supported concept means no request. |
...
| `EditorialConcept` (canonical result) | Child of one dossier/opportunity: subject/relationship/emotion, current hook, typed legacy connection, intro leads, handoff, three-to-six semantic montage beats, payoff, evidence, verification, structure-derived quality score, uncertainties, and its own concept-bound `FootageRequest` candidate. The immutable result preserves `Opportunity -> FandomStoryDossier -> EditorialConcept[] -> selected EditorialConcept -> FootageRequest`; relational evidence/source rows remain factual authority. Existing M1 relational rows remain backward-compatible, while the validated canonical result JSON owns the full M1.1b graph, so no destructive data migration is needed. |
| `recommendation_feedback` | Local-only research run, opportunity, optional concept, one bounded rating category, and created time. Feedback is evaluation/preference evidence only; M1.1 performs no automated training. |
